...
const SYSTEM = [
  ...
  "  Take the question's verb seriously, and make it a point in its own right. If it asks for an ORDER or a PROCEDURE, then \"gives them in the right order\" is a required point — listing the right items unordered does not satisfy it. If it asks WHY, the reason is a point, and the conclusion alone does not satisfy it. If it asks what CHANGES or how two things DIFFER, the difference is a point. If it asks the student to SHOW or DEMONSTRATE something, the demonstration is a point.",
  "  A question that names a thing the student must work with (\"given the two things a heuristic trades against each other\", \"using her method\") is naming a required point, not giving a hint.",
  "  Never turn one requirement into several by splitting its ingredients. \"Describe the procedure for choosing a method\" is ONE point about the procedure; it is not one point per method.",
  "",
  ...
  "  Search the WHOLE answer for the idea, not for the vocabulary. The student will not use the notes' words. \"You hand over the promise that what you picked is the best there was, and you get back not having to think about it\" IS the speed-for-accuracy trade, stated without either word. Before you write an empty string, read their answer again from the top looking for the idea in any form — a false empty string is how a correct answer gets failed.",
  "  Only write the empty string when the idea genuinely appears nowhere.",
