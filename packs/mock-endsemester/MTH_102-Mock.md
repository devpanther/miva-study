# MTH_102 — End-semester Mock

*Elementary Mathematics II (Calculus) · Weeks 5–9 · 75 minutes · notes closed · 36 marks*
...
b) 6x + 2
c) 6xh + 3h² + 2h
d) it cannot be simplified: at h = 0 the quotient is 0/0

**2.** If y = 3√x − 2/x + 7, then dy/dx =
a) 3/(2√x) + 2/x²
b) 3/(2√x) − 2/x²
c) 3/(2√x) + 2/x² + 7
d) 3/(2√x) + 2/x
...
**6.** For the curve 2x² + xy − y² = 8, dy/dx =
a) (4x + y)/(2y − x)
b) (4x + y)/(2y)
c) (4x + y)/(x − 2y)
d) 2x/y

**7.** If y = 2x⁴ − 5x³ + 7x − 1, then d²y/dx² =
a) 24x² − 30x
b) 8x³ − 15x² + 7
c) 48x − 30
d) 24x² − 30x + 7
...
d) f is decreasing, so f″(x) < 0

**10.** For f(x) = x³ − 6x² + 5:
a) local maximum at (0, 5), local minimum at (4, −27), inflection at (2, −11)
b) local minimum at (0, 5), local maximum at (4, −27), inflection at (2, −11)
c) local maximum at (0, 5), local minimum at (4, −27), inflections at x = 0 and x = 4
d) stationary points at x = 2 only, since f″(2) = 0

**11.** f(x) = x⁴ has f″(0) = 0. It follows that:
a) x = 0 is a local minimum; f″(c) = 0 only makes c a **candidate** inflection, and the concavity must be checked for a change of sign
b) x = 0 is a point of inflection, since f″(0) = 0
c) x = 0 is neither a maximum nor a minimum, the second-derivative test being inconclusive there
d) f has no stationary point at 0, since f″(0) = 0

**12.** A particle moves with s(t) = t³ − 6t² + 9t + 2 metres, t ≥ 0 seconds. On the interval 1 < t < 2 the particle is:
a) moving in the negative direction and speeding up
b) moving in the negative direction and slowing down, since the acceleration is negative
c) at rest, since the acceleration is zero somewhere in the interval
...
**18.** For y = x² − 4 on [0, 3]:
a) ∫₀³(x² − 4)dx = −3, while the total area enclosed between the curve and the x-axis is 23/3
b) both the integral and the total area equal 23/3
...
d) substitution in both cases

**20.** ∫(2x + 3)/(x² + 3x + 7)dx =
a) ln|x² + 3x + 7| + C
b) (1/2)ln|x² + 3x + 7| + C
c) (2x + 3)·ln|x² + 3x + 7| + C
d) (x² + 3x)/(x³/3 + 3x²/2 + 7x) + C
...
**23.** To evaluate ∫dx/√(x² − 9) the correct substitution is:
a) x = 3 secθ   b) x = 3 sinθ   c) x = 9 secθ   d) x = 3 tanθ

**24.** The correct partial-fraction form for (2x² + 3)/[(x − 1)²(x² + 4)] is:
a) A/(x − 1) + B/(x − 1)² + (Cx + D)/(x² + 4)
b) A/(x − 1)² + (Bx + C)/(x² + 4)
c) A/(x − 1) + B/(x − 1)² + C/(x² + 4)
d) A/(x − 1) + B/(x − 1)² + C/(x − 2) + D/(x + 2)

**25.** ∫₁^∞ dx/x³ equals:
a) 1/2   b) 1/3   c) 1   d) it diverges

**26.** Which one of the following **converges**?
a) ∫₀¹ dx/√x   b) ∫₁^∞ dx/√x   c) ∫₀¹ dx/x   d) ∫₁^∞ dx/x

**27.** ∫₀^∞ dx/√x:
a) diverges — it is improper at both ends, and although the piece on (0, 1] converges, the piece on [1, ∞) does not
b) converges to 2
...
**29.** ∫₁^∞ dx/√(x⁴ + 1):
a) converges, since 0 < 1/√(x⁴ + 1) < 1/x² on [1, ∞) and ∫₁^∞ dx/x² converges
b) diverges, since √(x⁴ + 1) > x² makes 1/√(x⁴ + 1) > 1/x²
c) converges, and its value is 1
d) cannot be decided, since the integrand has no elementary antiderivative

**30.** Suppose 0 ≤ f(x) ≤ g(x) for all x ≥ 1 and ∫₁^∞ f(x)dx converges. Then ∫₁^∞ g(x)dx:
a) may converge or may diverge — this ordering carries no information
b) converges
c) diverges
d) converges, to a value at least as large as that of ∫₁^∞ f
...
**33.** Evaluate, showing every step and **naming the technique** used each time:
(a) ∫(3x² − 4/x² + 5/x)dx
(b) ∫₀² x(x² + 3)²dx, changing the limits with the substitution
(c) For y = x² − 4 on [0, 3], find both ∫₀³(x² − 4)dx and the **total area** enclosed between the curve and the x-axis, and explain why the two numbers differ
(d) State Part 1 of the Fundamental Theorem and use it to write down d/dx[∫₂^x √(t³ + 1)dt] and d/dx[∫₂⁵ √(t³ + 1)dt]

**34.** Evaluate, in each case **naming the technique and saying why it is the one the shape of the integrand calls for**:
(a) ∫x²ln x dx
(b) ∫eˣ sin x dx
(c) ∫(x³ + x)/(x² − 1)dx
...
**6. a** — *Implicit differentiation: every y-term produces a dy/dx, and xy needs the product rule*: 4x + (y + x·y′) − 2y·y′ = 0 ⟹ y′(x − 2y) = −(4x + y) ⟹ y′ = **(4x + y)/(2y − x)**. (b) applies the product rule to xy but then loses the x·y′ term when collecting; (c) mis-signs the transposition, dividing by (x − 2y) instead of (2y − x); (d) differentiates xy as if it were a constant and y² as 2y with no dy/dx. **[Week 5]**

**7. a** — *Differentiate twice, in order*: y′ = 8x³ − 15x² + 7, then y″ = **24x² − 30x**. (b) stops at the first derivative; (c) is y‴; (d) carries the constant 7 into the second derivative, but d/dx(7) = 0. **[Week 6]**
...
**11. a** — *f″(c) = 0 is necessary but not sufficient for an inflection*: f″ = 12x² is ≥ 0 on **both** sides of 0, so the concavity never changes; f′ = 4x³ goes from − to +, so x = 0 is a **local minimum**. (b) treats the necessary condition as sufficient — the single most repeated inflection error; (c) states the test's verdict correctly but then wrongly concludes there is no extremum, whereas "inconclusive" means fall back on the sign of f′, which here settles it; (d) confuses f″(0) = 0 with f′(0) = 0, and in fact f′(0) = 0 too. **[Week 6]**

**12. a** — *Speeding up ⟺ v and a have the same sign*: v = 3t² − 12t + 9 = 3(t − 1)(t − 3), so v < 0 on (1, 3); a = 6t − 12 < 0 on (1, 2). Both negative ⟹ **speeding up, backwards**. (b) is the standard misreading, "negative acceleration means slowing down" — true only when v > 0; (c) confuses a = 0 (at t = 2) with "at rest", which is v = 0; (d) gets both signs wrong. **[Week 6]**

**13. a** — *Power rule for integration, plus the excluded case n = −1*: 6x²→2x³; −4x⁻³ → −4·x⁻²/(−2) = **+2x⁻²**; 1/x → **ln|x|**. (b) misses that two minus signs make a plus; (c) has **differentiated** the middle term instead of integrating it; (d) drops the + C — an indefinite integral is the whole family, so this is a wrong answer, not an untidy one. **[Week 7]**
...
**21. a** — *Parts, u by LIATE (A beats T, so u = x)*: u = x, dv = cos x dx ⟹ v = sin x, giving x sin x − ∫sin x dx = **x sin x + cos x + C**. (b) forgets that ∫sin x dx = −cos x, so the two minus signs make a plus; (c) integrates the two factors separately — ∫fg ≠ (∫f)(∫g); (d) mis-signs the uv term. Check (a) by differentiating: sin x + x cos x − sin x = x cos x ✓. **[Week 8]**

**22. a** — *The dv = dx trick*: u = ln x, dv = dx ⟹ du = dx/x, v = x, so ∫ln x dx = x ln x − ∫x·(1/x)dx = **x ln x − x + C**. (b) differentiates instead of integrating; (c) treats ln x as if it were u with du = dx, i.e. applies the power rule to a function that is not a power of the variable of integration; (d) mis-signs the second term — differentiate it and you get ln x + 2, not ln x. **[Week 8]**

**23. a** — *A minus with x² **first** (x² − a²) calls for x = a secθ*, on which sec²θ − 1 = tan²θ clears the root. (b) is the substitution for a² − x²; (c) uses a² = 9 in place of **a** = 3 — the substituted constant is the square root of the constant term; (d) is for a² + x². Getting this choice wrong leaves a root that refuses to simplify. **[Week 8]**
...
**Speeding up** ⟺ v and a share a sign. On [0, 1): v < 0 and a < 0 ⟹ speeding up (backwards). On (1, 3): v < 0, a > 0 ⟹ slowing down. On (3, ∞): v > 0, a > 0 ⟹ speeding up. So **speeding up on [0, 1) ∪ (3, ∞)**.
— *Iterated product rule and the cross term; the nth-derivative pattern; "at rest" vs "acceleration zero", and the same-sign criterion.* **[Week 6]**

**33.**
(a) **Rewrite, then the power rule term by term, with n = −1 as the excluded case**: ∫(3x² − 4x⁻² + 5x⁻¹)dx = x³ − 4·x⁻¹/(−1) + 5 ln|x| + C = **x³ + 4/x + 5 ln|x| + C**. Check: 3x² − 4x⁻² + 5/x ✓. The modulus in ln|x| is compulsory — 1/x is defined for negative x too.
(b) **Substitution with the limits changed**: u = x² + 3, du = 2x dx ⟹ x dx = du/2; x = 0 ⟹ u = 3, x = 2 ⟹ u = 7.
∫₀²x(x² + 3)²dx = (1/2)∫₃⁷u²du = (1/2)[u³/3]₃⁷ = (1/6)(343 − 27) = **158/3**.
(Either change the limits, as here, or revert to x and use 0 and 2 — never evaluate a u-antiderivative at the x-limits.)
(c) The curve crosses the axis at x = 2, inside [0, 3].
∫₀³(x² − 4)dx = [x³/3 − 4x]₀³ = 9 − 12 = **−3** — the **net signed** area.
Piecewise: ∫₀²= 8/3 − 8 = −16/3 (below the axis); ∫₂³ = (9 − 12) − (8/3 − 8) = 7/3 (above). **Total area = 16/3 + 7/3 = 23/3.**
They differ because the integral counts area below the axis as **negative**, so the two pieces partly cancel; total area adds the **absolute values** of the pieces.
(d) **FTC Part 1:** d/dx[∫ₐ^x f(t)dt] = f(x) — differentiating an accumulation returns the rate. So d/dx[∫₂^x √(t³ + 1)dt] = **√(x³ + 1)**, while d/dx[∫₂⁵ √(t³ + 1)dt] = **0**, the second being a fixed number and not a function of x.
— *Rewrite-then-integrate; substitution with transformed limits; signed versus total area; the accumulation form of the FTC.* **[Week 7]**

**34.**
(a) A product of two unrelated types with a logarithm present, so **integration by parts with u chosen by LIATE (L beats A)**: u = ln x, dv = x²dx ⟹ du = dx/x, v = x³/3.
∫x²ln x dx = (x³/3)ln x − ∫(x³/3)(1/x)dx = (x³/3)ln x − (1/3)∫x²dx = **(x³ ln x)/3 − x³/9 + C**.
*Check:* d/dx[(x³ln x)/3 − x³/9] = x²ln x + (x³/3)(1/x) − x²/3 = x²ln x + x²/3 − x²/3 = **x²ln x** ✓.
(b) An exponential against a trigonometric function — neither simplifies on differentiation, so this is the **cycling** case: parts twice, keeping the exponential as dv **both** times, then solving for I.
I = ∫eˣsin x dx: u = sin x, dv = eˣdx ⟹ I = eˣsin x − ∫eˣcos x dx.
For J = ∫eˣcos x dx: u = cos x, dv = eˣdx ⟹ J = eˣcos x + ∫eˣsin x dx = eˣcos x + I.
So I = eˣsin x − eˣcos x − I ⟹ 2I = eˣ(sin x − cos x) ⟹ **I = eˣ(sin x − cos x)/2 + C**.
(Reversing the roles at the second application returns the useless identity I = I.)
(c) A **ratio of polynomials that is improper** (deg 3 ≥ deg 2), so **long-divide first**, then partial fractions: x³ + x = x(x² − 1) + 2x, hence
(x³ + x)/(x² − 1) = x + 2x/((x − 1)(x + 1)). Setting 2x = A(x + 1) + B(x − 1): x = 1 ⟹ A = 1; x = −1 ⟹ B = 1.
∫ = ∫[x + 1/(x − 1) + 1/(x + 1)]dx = **x²/2 + ln|x − 1| + ln|x + 1| + C = x²/2 + ln|x² − 1| + C**.
(Forcing A/(x − 1) + B/(x + 1) on the undivided fraction gives an unsolvable system.)
— *LIATE and the parts trade; the cycling integral solved algebraically; properness check, long division and partial fractions.* **[Week 8]**
...
Since (ln t)/t → 0 as t → ∞, ∫₁^∞(ln x)/x² dx = **1 — converges**.
For the second, **substitution** u = ln x, du = dx/x: ∫₁^t (ln x)/x dx = [(ln x)²/2]₁^t = (ln t)²/2 → ∞, so ∫₁^∞(ln x)/x dx **diverges**.
What separates them: the logarithm grows more slowly than **any** positive power of x, so it cannot rescue the divergent tail 1/x (p = 1, the boundary) but it also cannot spoil the convergent tail 1/x² (p = 2 > 1). The decisive quantity is the **power of x in the denominator**, not the logarithm.
— *Every improper integral is a limit of a proper one; parts and partial fractions are the Week 8 machinery carried out inside that limit; the p = 1 boundary decides the pair in (c).* **[Weeks 8–9]**

---

## What to revise

Coverage is deliberately even — seven or eight of the thirty-six marks per week, with Q36 straddling Weeks 8 and 9.
