...
**12.** A student evaluates ∫₀³ 6x dx as 6x|₀³ = 18 − 0 = 18. What went wrong, and what is the correct value?
A. The constant of integration was dropped; the value is 18 + c
B. The limits went into the integrand rather than the antiderivative 3x²; the value is 27
C. The limits went into the integrand rather than the antiderivative 6x²; the value is 54
D. Nothing went wrong; the value is 18
...
**9. B** — *Integrating a polynomial term by term.* Split and apply the power rule to each term: ∫2x² dx = 2x³/3, ∫3x dx = 3x²/2, ∫4 dx = 4x. So the integral is (2/3)x³ + (3/2)x² − 4x + c, and differentiating gives back 2x² + 3x − 4.

2x³ + 3x² − 4x + c raises each power but never divides by it; (2/3)x³ + (3/2)x² − 4 + c leaves the constant term unintegrated; (1/3)x³ + (1/2)x² − 4x + c divides correctly but drops the coefficients 2 and 3.

**10. A** — *Definite integral with a cubed negative limit.* An antiderivative is F(x) = x³ − 2x. At x = 1: 1 − 2 = −1. At x = −2: (−2)³ − 2(−2) = −8 + 4 = −4. So the integral is F(1) − F(−2) = −1 − (−4) = 3.

−13 comes from taking (−2)³ as +8, giving F(−2) = 12; 11 comes from writing −2(−2) = −4, giving F(−2) = −12; −5 adds F(1) and F(−2) instead of subtracting.

**11. D** — *Dividing before integrating a definite integral.* Divide each term by x² first: (x² − 1)/x² = 1 − x⁻². An antiderivative is F(x) = x + 1/x, since ∫x⁻² dx = x⁻¹/(−1) = −1/x and subtracting it gives +1/x. Then F(2) − F(1) = 5/2 − 2 = 1/2.

3/2 uses x − 1/x, losing that sign; 4/3 integrates x² − 1 without dividing by x²; −1/2 subtracts the limits in the wrong order.

**12. B** — *Substituting limits into the antiderivative, not the integrand.* The Fundamental Theorem reads ∫ₐᵇ f(x) dx = F(b) − F(a), where F is an antiderivative of f. The student substituted into f(x) = 6x itself. Integrating first gives F(x) = 3x², so the value is 3(3²) − 3(0²) = 27.

6x² is what you get by raising the power without dividing by 2, and gives 54; a definite integral never carries + c, since the constant cancels; and 18 is the value of the integrand at the limits, not of the integral.
